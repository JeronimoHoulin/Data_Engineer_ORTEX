...
df = df[[
    "exchange", "inputdate", "companyName", "valueEUR", "tradeSignificance"
    ]]
...
cursor.execute(sql)
resp = cursor.fetchall()
df_august = pd.DataFrame(resp, columns=[
    'exchange', 
    'date', 
    'name', 
    'eur', 
...
sum_company = df_august.groupby(
    ["name"]
    ).sum().sort_values(
        ["eur"], ascending=False).reset_index()
top2 = sum_company[0:2]

print("\n")
print("Question 2")
print("Companies with highest ValueEUR in August:")
print(f" #1: {top2.name[0]} with ${'{:,.2f}'.format(round(top2.eur[0], 1))}.")
print(f" #2: {top2.name[1]} with ${'{:,.2f}'.format(round(top2.eur[1], 1))}.")
...
df_3 = pd.DataFrame(resp, columns=[
    'exchange',
    'date',
    'name',
    'eur',
...
"""
I will use Value EUR as the referance for transaction volume, 
as it will be displayed in %.. so it doesn't matter if its USD
"""
